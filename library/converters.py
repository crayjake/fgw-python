...
def shallowPhi(z, j, h, D, Dz, A):
    return A * np.sin((j * np.pi * z) / Dz)

def shallowPhiPrime(z, j, h, D, Dz, A):
    return A * ((j * np.pi) / D) * np.cos((j * np.pi * z) / Dz)

def shallowRhoZero(z, rho_s, h, D, Dz):
    return rho_s




def shallowConverter(inp: State, meta: Meta, mode: int = -1) -> np.ndarray:
    start = 0
    ...
    for i in range(len(meta.js)):
        ...
        for z in range(meta.x.shape[0]):
            # phi like
            w[z, :]   += (inp.w[i]
                         * shallowPhi(z, meta.js[i], h, meta.D, meta.x.shape[0]-1, A_j))
            rho[z, :] += (inp.rho[i]
                         * ((shallowRhoZero(z, rho_s, h, meta.D, meta.x.shape[0]-1) * (N ** 2)) / (c_jSquared))
                         * shallowPhi(z, meta.js[i], h, meta.D, meta.x.shape[0]-1, A_j))
            b[z, :]   += (inp.b[i] * ((N ** 2) / c_jSquared)
                         * shallowPhi(z, meta.js[i], h, meta.D, meta.x.shape[0]-1, A_j))
            ...
            # phi' like
            u[z, :]   += (inp.u[i]
                         * shallowPhiPrime(z, meta.js[i], h, meta.D, meta.x.shape[0]-1, A_j))
            v[z, :]   += (inp.v[i]
                         * shallowPhiPrime(z, meta.js[i], h, meta.D, meta.x.shape[0]-1, A_j))
            p[z, :]   += (inp.p[i] * shallowRhoZero(z, rho_s, h, meta.D, meta.x.shape[0]-1)
                         * shallowPhiPrime(z, meta.js[i], h, meta.D, meta.x.shape[0]-1, A_j))
